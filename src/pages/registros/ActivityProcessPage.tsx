...
export const ActivityProcessPage = () => {
    ...
    const [yorden, setYorden] = useState<number>(0)
    ...
    const getProcesos = async () => {
        const url = `${baseUrl}/listaunproceso?id_proceso=${process_id}`
        try {
            const respuesta = await fetch(url);
            const data = await respuesta.json();
            if (data.exito === "SI") {
                setProcesos(data.registros);
                selectProcess(data.registros[0].id, data.registros[0].name)
            } else {
                Swal.fire({
                    title: "Error",
                    text: data.mensaje,
    ...
    const getActividadesactuales = async () => {
    ...
    const getActividades = async () => {
        const url = `${baseUrl}/listaunaactividad?id_proceso=${process_id}`
        try {
            console.log(url);
            const respuesta = await fetch(url);
            const data = await respuesta.json();
                ...
                >
                    <MenuItem value={0}>0</MenuItem>
                    {
                        selectedActivities && selectedActivities.map((sa, i) => (
                            ...
                            : <MenuItem value={activity.orden} key={sa.id}>{activity.orden}</MenuItem>*/
                        ))
                    }
                </TextField>
            </Box>
