...
export const ProcessesPageCopy: FC = () => {
   const { process_id } = useParams();
    ...
    const getProceso = async () => {
      const url = `${baseUrl}/listaprocesos?id_proceso=${process_id}`
      try {
          const respuesta = await fetch(url);
          ...
          if (data.exito === "SI") {
            ...
            setProcess(data.registros[0].name)
            setCategoria(data.registros[0].categoria)
            setDescripcion(data.registros[0].description)
            setCentroDeCostoUno(data.registros[0].centrodecosto1)
            setCentroDeCostoDos(data.registros[0].centrodecosto2)
            // setProceso(data.registros)
          } else {
              setProceso(null);
              console.log("No se logro encontrar ningún usuario")
          }
