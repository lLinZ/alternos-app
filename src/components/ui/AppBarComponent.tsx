...
const pages: Pages[] = [{ name: "Dashboard", path: "/dashboard" }];
const settings: Settings[] = [
    { name: "Administrar usuarios", path: "/admin" },
    { name: "Procesos y actividades", path: "/process" },
    { name: "Registrar procesos", path: "/process/add" },
                            ...
                            <Box sx={{ flexGrow: 0 }}>
                                <Tooltip title="Abrir configuraciones">
                                    <IconButton onClick={handleOpenUserMenu} sx={{ p: 0 }}>
                                        <Avatar alt={user?.name || "Nombre de usuario"}>{user?.name.substring(0, 1) || "U"}</Avatar>
                                    </IconButton>
                                </Tooltip>
                                ...
                                >
                                    {token &&
                                        (user && user.role_name === "Administrador"
                                            ? settings.map(
                                                (setting) => String(currentPath.pathname) !== String(setting.path) && (
